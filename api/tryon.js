export default async function handler(req, res) {
  ...
  try {
      ...
      const pr = await fetch('https://api.replicate.com/v1/predictions', {
        ...
        body: JSON.stringify({
          ...
          input: {
            human_img: humanUrl,
            garm_img: garmentUrl,
            garment_des: garmentDes || 'clothing',
    ...
    for (var s = 0; s < seeds.length; s++) {
      try {
        var out = await runSeed(seeds[s]);
        outputs.push(out);
        console.log('seed', seeds[s], 'ok:', out.slice(-30));
        if (s < seeds.length - 1) await sleep(500);
      } catch(e) {
        console.warn('seed', seeds[s], 'failed:', e.message);
      }
    }
    ...
    if (outputs.length > 1 && ANT) {
      try {
        ...
        var content = [];
        content.push({ type:'image', source:{ type:'url', url: humanUrl } });
        content.push({ type:'text', text: 'Original person' });
        content.push({ type:'image', source:{ type:'url', url: garmentUrl } });
        content.push({ type:'text', text: 'Reference garment' });
        ...
        content.push({ type:'text', text: 'Which best preserves the person face AND shows the garment? Reply: 1, 2, or 3 only.' });
        var cr = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 3,
            system: 'Reply with only a single digit.', messages: [{ role:'user', content: content }] })
        });
        ...
        console.warn('Claude pick failed:', e.message);
      }
    }
