...
';
    ...
    var hs  = '--' + bnd + CR + 'Content-Disposition: ' + disp + CR + 'Content-Type: ' + ct + CR + CR;
    var ts  = CR + '--' + bnd + '--' + CR;
  ...
  try {
    ...
    if (!humanUrl && body.humanBase64)   humanUrl   = await uploadB64(body.humanBase64,   'person.jpg',  R8);
    if (!garmentUrl && body.garmentBase64) garmentUrl = await uploadB64(body.garmentBase64, 'garment.jpg', R8);
    ...
    for (var s = 0; s < seeds.length; s++) {
      try {
        var out = await runSeed(humanUrl, garmentUrl, garmentDes, category, seeds[s], R8);
        outputs.push(out);
        if (s < seeds.length - 1) await sleep(500);
      } catch(e) { console.warn('seed ' + seeds[s] + ':', e.message); }
    }
    if (outputs.length === 0) throw new Error('All seeds failed. Check balance at replicate.com/account/billing');
    ...
    if (outputs.length > 1 && ANT) {
      try {
        ...
        content.push({ type: 'text', text: 'Which looks most natural and photorealistic? Reply only: 1, 2, or 3.' });
