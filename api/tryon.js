export default async function handler(req, res) {
  ...
  var uploadB64 = async function(dataUrl, filename, R8) {
    ...
    var ts  = NL + '--' + bnd + '--' + NL;
    var body = Buffer.concat([Buffer.from(hs), buf, Buffer.from(ts)]);
  ...
  var createPred = async function(hUrl, gUrl, des, cat, seed, R8) {
  ...
  var pollPred = async function(predId, R8) {
    for (var i = 0; i < 25; i++) {
      await sleep(3000);
      var p = await (await fetch('https://api.replicate.com/v1/predictions/' + predId,
